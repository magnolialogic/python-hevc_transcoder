...
for h264_file in h264_files:
	h264_file_path = os.path.relpath("h264/{filename}".format(filename=h264_file))
	h264_file_size = int(os.path.getsize(h264_file_path) / 1000000)
	h264_file_handle = cv2.VideoCapture(h264_file_path)
	hevc_files = [filename for filename in os.listdir("hevc") if filename.startswith(os.path.splitext(h264_file)[0])]
	...
	for hevc_file in hevc_files:
		...
		compression_ratio = int(100-(hevc_file_size/h264_file_size*100))
		total_frames = h264_file_handle.get(cv2.CAP_PROP_FRAME_COUNT)
		stride = int(total_frames / (args.num_frames + 1))

		print("\nFilename:\t\t{filename}".format(filename=hevc_file))
		if h264_file_handle.get(cv2.CAP_PROP_FRAME_COUNT) != hevc_file_handle.get(cv2.CAP_PROP_FRAME_COUNT):
			print("\t\t\t!!! WARNING: Frame counts do not match, screencaps may be time-shifted")
		print("\tH264 Size:\t{size} MB".format(size=h264_file_size))
		...
		with open(os.path.join(output_directory, "summary.txt"), "w") as summary_file:
			summary_file.write("{h264} MB to {hevc} MB, saving {ratio}%".format(h264=str(h264_file_size), hevc=str(hevc_file_size), ratio=str(compression_ratio)))
		for frame in range(1, args.num_frames+1):
			h264_file_handle.set(cv2.CAP_PROP_POS_FRAMES,stride*frame)
			hevc_file_handle.set(cv2.CAP_PROP_POS_FRAMES,stride*frame)
			ret,h264_frame = h264_file_handle.read()
			ret,hevc_frame = hevc_file_handle.read()
			if args.stack:
				comparison_frame = np.vstack((h264_frame,hevc_frame))
				cv2.imwrite(os.path.join(output_directory, "{number}-2up.png".format(number=frame)), comparison_frame)
			cv2.imwrite(os.path.join(output_directory, "{number}-original.png".format(number=frame)), h264_frame)
			cv2.imwrite(os.path.join(output_directory, "{number}-x265.png".format(number=frame)), hevc_frame)

		hevc_file_handle.release()

	h264_file_handle.release()
